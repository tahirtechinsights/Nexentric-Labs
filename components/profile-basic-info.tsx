...
export default function ProfileBio({ initialUser }: { initialUser: FullUser }) {
  ...
  const handleSave = () => {
    startTransition(async () => {
      // Create an object with the bio data instead of FormData
      const bioData = {
      ...
      // Pass the object directly (not wrapped in FormData)
      const { error } = await updateUserBio(user.id, bioData);
        ...
        <div className="space-y-2">
          ...
          {isEditing ? (
            <Input
              ...
              onChange={(e) =>
                handleBioChange("hourlyRate", parseFloat(e.target.value) || 0)
              }
              min="0"
              step="0.01"
            />
        ...
        <div className="space-y-2">
          ...
          {isEditing ? (
            <Textarea
              ...
              onChange={(e) =>
                handleBioChange("skillsAndTools", e.target.value)
              }
